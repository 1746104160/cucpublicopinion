...
LastEditors: 邵佳泓
LastEditTime: 2022-07-14 14:25:24
...
def create_db():
    ...
    vip = Roles(name='vip',
                ...
                authedroutes="['/personal','/news','/security','/log','/version']")
    admin = Roles(name='admin',
                  description='拥有系统的全部权限',
                  authedroutes="['/personal','/news','/security','/log','/version','/dashboard']")
    Roles.add(vip)
    Roles.add(admin)
    Roles.add(standard)
    ...
    user = Users(name="administrator",
                 ...
                 last_login_ip='127.0.0.1',
                 description="admin用户")
    user.set_password("kQ2wm1ZYqJNp")
    Users.add(user)
    for i in range(1, 10):
