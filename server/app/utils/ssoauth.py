...
@version: 1.0.0
@Author: 邵佳泓
@Date: 2022-07-04 16:28:40
@LastEditors: 邵佳泓
@LastEditTime: 2022-07-05 12:15:56
...
def loginsso(username: str, password: str) -> bool:
	...
	execution = re.findall(r'<input type="hidden" id="execution" name="execution" value="(.*)" />', login_html)[0]
	salt = re.findall(r'<input type="hidden" id="pwdEncryptSalt" value="(.*)" /><input ', login_html)[0]

	pwd = quote(encrypt(password, salt))

	url = "https://sso.cuc.edu.cn/authserver/login"

	payload = 'username=' + username + '&password=' + pwd + '&captcha=&_eventId=submit&cllt=userNameLogin&dllt=generalLogin&lt=&execution=' + execution
