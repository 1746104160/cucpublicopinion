...
@LastEditors: 邵佳泓
@LastEditTime: 2022-07-07 23:48:42
...
class UserIno(Resource):
	...
	def get(self, page: int, size: int, order: str):
		...
		if 'admin' not in [role.name for role in user.role]:
			...
			return {'code': 0, 'message': '获取用户信息成功', 'success': True, 'data': data, 'total': length}
		else:
			users = Users.query.order_by(Users.userid.asc() if order == 'ascending' else Users.userid.desc()).paginate(page, size, False).items
			data = [{
				'userid': user.userid,
				'name': user.name,
				'email': user.email,
...
updateparser.add_argument('role', type=str, nullable=False, required=True, help='角色不能为空',action='append')
