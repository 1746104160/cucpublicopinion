...
<template>
  <div class="form-container">
    <el-steps :active="active" finish-status="success" align-center>
      ...
      <el-step title="绑定中传SSO账号"></el-step>
    ...
    <template v-if="active == 0">
      <el-form
        ...
        class="reset-form"
      >
        <el-form-item label="验证邮箱" prop="account">
          <el-input
    ...
    <template v-if="active == 1">
      ...
      <el-form-item
        ...
        <el-form-item label="中传SSO账号" prop="cucaccount">
          <el-input
            v-model="resetForm.cucaccount"
            autocomplete="off"
            placeholder="请输入绑定邮箱"
          />
        </el-form-item>
        ...
        <el-form-item label="图片验证码" prop="captchapic">
          ...
          <el-button type="primary" @click="handleResetSSO">确认绑定中传SSO账号</el-button>
      </el-form>
    </template>
  ...
  setup () {
    ...
    const handleNextStep = () => {
      ...
      verifyRef.value.validate((valid: any) => {
        if (valid) {
          if (state.verifyForm.account !== JSON.parse(sessionStorage.getItem('userinfo') as string).email) {
            getNextStepFailure()
            return false
          } else {
     ...
     *
     */
    const handleResetSSO = async () => {
      resetRef.value.validate(async (valid: any) => {
        if (valid) {
          try {
            const { account } = state.verifyForm
            const { cucaccount, cucpassword, captchaemail, captchapic } = state.resetForm
            const data = {
              account,
              verify: captchaemail,
              cucaccount,
              password: encrypt(cucpassword),
              captcha: captchapic
            }
            Service.postResetSSO(data)
              .then((res) => {
    ...
    const validateCUCAccount = (rule: any, value: any, callback: any) => {
      if (!value) {
        callback(new Error('请输入中传SSO账号'))
    ...
    const resetrules = reactive<FormRules>({
      cucaccount: [{ required: true, trigger: 'blur', validator: validateCUCAccount }],
